...
select * from supplier where supp_id in (select supp_id from supplier_pricing group by  supp_id having count(supp_id)>2);
...
inner join 
(select sp.pro_id, ord_date from supplier_pricing sp inner join order_directory.order od on sp.pricing_id=od.pricing_id) pp
